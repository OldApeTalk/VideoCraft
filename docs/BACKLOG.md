...
| 优先级 | 状态 | 功能 | 说明 |
|--------|------|------|------|
| 🔴 P1 | [ ] | ClaudeCode provider：长期 token 入 Settings UI | 2026-08-22 实测发现：`claude -p` headless/subprocess 模式下 OAuth 刷新不如交互模式可靠，失败时报 `Failed to authenticate: OAuth session expired and could not be refreshed`（本机登录其实没过期，纯粹是无 TTY 的子进程刷新链路问题）。CLI 官方给的解法是 `claude setup-token` 生成的长期 token，通过 `CLAUDE_CODE_OAUTH_TOKEN` 环境变量喂给它（`claude.exe` 二进制里能搜到这个字符串，命名已核实）。**当前唯一可行的绕过办法是用户自己手搓 Windows 环境变量（PowerShell `[Environment]::SetEnvironmentVariable` + 完全重启 App）**——普通用户完全不可能自己走通，还很容易把示例里的占位符尖括号 `< >`一起抄进值里（实测踩过）。**要做的**：Settings/AI 面板给 `ClaudeCode` provider 加一个可选输入框（复用 Gemini/DeepSeek 那套"填 key → 存 `user_data/keys/`"UI 和存储机制），填了就在 [`core/ai/providers/claude_code.py`](../src/core/ai/providers/claude_code.py) 的 `_run()` spawn 子进程时注入成 `CLAUDE_CODE_OAUTH_TOKEN` 环境变量；不填维持现状（吃本机 CLI 登录态）。生成 token 本身那步（`claude setup-token`，需要浏览器登录确认）没法在 App 内自动化，用户仍需自己跑一次命令、把结果粘贴进来——这个降不掉，只降低"设置环境变量"这后半段的操作门槛。顺带：`_run()` 的报错信息之前 stderr 为空时会吞掉 stdout 报出无意义的 `<no stderr>`，以及 auth 类错误关键词嗅探漏掉了 "authenticate"/"oauth" 这两个真实文案——这两处诊断性小修已经在本地改完（未提交），下次动这块代码时一并带上。 |
