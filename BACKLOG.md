...
| 🔴 P1 | [ ] | AI Router Manager 改为 Hub 内 tab | 当前 `router_manager.RouterManagerWindow` 是独立 `tk.Toplevel` + `grab_set` 弹窗，与 Hub 内所有主功能（SubtitleTool 等）"tab 内打开"的范式不一致。改造为 `ToolBase` 风格的 tab 页，通过 Hub `_open_in_tab()` 路径注册。call sites（[VideoCraftHub.py:372](src/VideoCraftHub.py#L372)、[translate_srt.py:334](src/tools/translate/translate_srt.py#L334)、[srt_tools.py:1180](src/tools/subtitle/srt_tools.py#L1180)、[text2video.py:56](src/tools/text2video/text2video.py#L56)）都要切成"激活 / 新建 router tab" |
