...
def refine_segment_descriptions(paragraphs_path, prompt, api_key_path=None, model_name='gemini-2.5-flash-lite'):
    """
    Refine each segment generated by Tab 3 while preserving the original timestamp/title.

    Args:
        paragraphs_path (str): Input paragraph file path (output from Tab 3).
        prompt (str): Refinement prompt with placeholders: {segment_time}, {segment_title}, {segment_content}.
        api_key_path (str): Gemini API key file path.
        model_name (str): Gemini model name.

    Returns:
        str: Refined segment text in "timestamp title + refined content" format.
    """
    ...
    # Process each segment independently for better control and fault visibility.
    for segment in segments:
        segment_content = segment['content']

        if not segment_content or segment_content == '(此时间段内无字幕内容)':
            refined_text = '（此时间段暂无可提炼内容）'
        else:
            full_prompt = (
                prompt
                .replace('{segment_time}', segment['time_str'])
                .replace('{segment_title}', segment['title'])
                .replace('{segment_content}', segment_content)
            )
            ...
            try:
                ...
                raise RuntimeError(f"分段 {segment['time_str']} {segment['title']} 调用Gemini失败: {e}")

            # Normalize multi-line model output into one concise line per segment.
            refined_text = ' '.join(
                line.strip(' -•\t')
            ...
            # Enforce the per-segment length limit (<= 128 chars).
            if len(refined_text) > 128:
                refined_text = refined_text[:128].rstrip('，。！？；：,.!?;: ')

            if not refined_text:
                refined_text = '（提炼结果为空）'

        refined_results.append(f"{segment['time_str']} {segment['title']}\n{refined_text}")
...
】"""
    ...
    def create_refine_tab(self):
        ...
        default_refine_prompt = """## 精炼每个分段

【
对每个分段内容进行总结提炼，每个段落提炼后不超过128个字。
